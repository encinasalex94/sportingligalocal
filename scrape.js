...
const CONFIG = {
  ...
  // Cada tanda de partidos de grupo de la Copa vive en una "competición" NFG
  // distinta (así lo modela su software), y la fase final es otra aparte
  // pero con las 4 rondas (octavos/cuartos/semis/final) en un único documento.
  copaStages: [
    { key: 'grupos', label: 'Fase de grupos', codCompeticion: '1009598', codGrupo: '1010292', groupPhase: true },
    { key: 'ronda4', label: '4º partido (grupos)', codCompeticion: '1009607', codGrupo: '1009635', groupPhase: true },
    { key: 'ronda5', label: '5º partido (grupos)', codCompeticion: '1010502', codGrupo: '1010515', groupPhase: true },
    { key: 'final', label: 'Fase final', codCompeticion: '1010540', codGrupo: '1010542', knockout: true },
  ],
};
...
async function main() {
  ...
  const copaStagesData = [];
  const groupPhaseRoundSets = [];
  for (const stage of CONFIG.copaStages) {
    try {
      const rounds = await fetchCalendario(stage.codCompeticion, stage.codGrupo, { knockout: !!stage.knockout });
      await sleep(REQUEST_DELAY_MS);

      // Hora y campo para TODOS los partidos de Copa (son pocos en total).
      for (const r of rounds) {
        try {
          const details = await fetchRoundDetail(stage.codCompeticion, stage.codGrupo, CONFIG.codTemporada, r.round);
          mergeRoundDetail(r, details);
          await sleep(REQUEST_DELAY_MS);
        } catch (err) {
          log(`  -> ${stage.label} ronda ${r.round}: no se pudo obtener hora/campo (${err.message})`);
        }
      }

      const ownMatches = extractOwnMatches(rounds);
      copaStagesData.push({ key: stage.key, label: stage.label, knockout: !!stage.knockout, matches: ownMatches });
      if (stage.groupPhase) groupPhaseRoundSets.push(rounds);
  ...
  const copaGroupStandings = groupPhaseRoundSets.length ? computeStandingsFromRounds(groupPhaseRoundSets) : [];
  ...
  const data = {
    ...
    copa: { stages: copaStagesData, groupStandings: copaGroupStandings },
  };
